...
frame_dir_list = glob(args.input+"/frame*")
out_frames = []
if not os.path.isdir(args.output):
    os.mkdir(args.output)
outvideo = os.path.join(args.output, "optimization.mp4")
...
for i in range(len(frame_dir_list)):
    ...
    square_im = square_im.astype(np.uint8)
    ...
    try:
        ...
        # out_frames.append(square_im)
        print("No person mask available.")
        person_detect = False

    if masks_person.size == 0:
        # out_frames.append(square_im)
        print("No person mask available.")
        person_detect = False
...
# cv2.imwrite("draw_masks_person.png", draw_masks_person*255)
    ...
    try:
        ...
        # out_frames.append(square_im)
        print("No plane mask available.")
        object_detect = False

    if masks_object.size == 0:
        # out_frames.append(square_im)
        print("No plane mask available.")
        object_detect = False
...
# cv2.imwrite("draw_masks_object.png", draw_masks_object*255)
    ...
    if person_detect and object_detect:
    ...
    if object_detect and not person_detect:
        verts = mesh1.verts_packed()
        faces = mesh1.faces_packed()
        mesh_color = create_mesh_texture(mesh1,colors['light_gray'],device)
        object_textures = Textures(verts_rgb = mesh_color)
        mesh = Meshes(verts=verts.unsqueeze(0), 
                faces=faces.unsqueeze(0),
                textures=object_textures
                )

    if person_detect and not object_detect:
        verts = mesh2.verts_packed()
        faces = mesh2.faces_packed()
        mesh_color = create_mesh_texture(mesh2,colors['light_purple'],device)
        person_textures = Textures(verts_rgb = mesh_color)
        mesh = Meshes(verts=verts.unsqueeze(0), 
                faces=faces.unsqueeze(0),
                textures=person_textures
                )
    ...
    if person_detect and object_detect:
        ...
        with torch.no_grad():
            opt_result = save_out(new_mesh, renderer, combine_opt, device, True, square_im)
            out_frames.append(opt_result.astype(np.uint8))
    elif person_detect or object_detect:
        opt_result = save_out(mesh, renderer, combine_initial, device, True, square_im)
        out_frames.append(opt_result.astype(np.uint8))
    else:
        out_frames.append(square_im)

print("Total frames of the video:", len(out_frames))
gen_video_out(outvideo, the_fps=10, img_array=out_frames)